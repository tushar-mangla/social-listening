...
def _compute_next_retry(status: Optional[str], attempts: int, now: datetime) -> Optional[datetime]:
    if status in ("provider_error", "unclassified"):
        try:
            from listening_loop import config as _config
...
def add_lead(lead: dict) -> bool:
    ...
    # Normalize summary/one_line pair (both persisted for compatibility).
    summary_val = lead.get("summary") if lead.get("summary") else lead.get("one_line")
    one_line_val = lead.get("one_line") if lead.get("one_line") else lead.get("summary")
    ...
    with get_db_connection() as conn:
        ...
        # Initialize before any status branch so the qualified +
        # qualified re-affirmation path never references it unbound.
        classified_at = now if incoming_status else None
        ...
        # Qualified + qualified re-affirmation: preserve every established
        # non-null qualification evidence field (and synced_to_notion_at,
        # which this UPDATE never touches). Only coalesce previously
        # missing/None fields from the incoming outcome.
        if existing and existing.get("classifier_status") == "qualified" and incoming_status == "qualified":
            def _keep(field: str):
            ...
            cursor.execute(
                """UPDATE leads SET
                    ...
                    classifier_status=?, icp=?, author_role=?, intent_type=?, urgency=?,
                    confidence=?, icp_score=?, intent_score=?, summary=?, one_line=?, keyword_score=?, rejection_reason=?, outreach_draft=?,
        ...
        if inserted:
            ...
            # Fill-missing enrichment: keep existing qualified-adjacent values
            # when the incoming outcome omits them, otherwise take incoming.
            def _pick(field: str):
                incoming = lead.get(field) if field not in ("summary", "one_line") else None
                if field == "summary":
...
def get_due_for_retry(limit: int = 64) -> list[dict]:
    """Return provider_error/unclassified rows due for bounded retry."""
    try:
        from listening_loop import config as _config
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        # Older rows may lack the new columns if migration has not run;
        # ensure schema first (idempotent, cheap).
        cols = {row[1] for row in cursor.execute("PRAGMA table_info(leads)")}
        if "next_classification_retry_at" not in cols or "classification_attempts" not in cols:
            conn.commit()
    initialize_db()
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """SELECT * FROM leads
               WHERE classifier_status IN ('provider_error', 'unclassified')
